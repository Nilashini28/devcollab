    ...
    <div style={{ padding: 32 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: 4 }}>Here's what's happening with your projects</p>
        </div>
      ...
      {workspaces.length === 0 ? (
        ...
        <div style={{ display: 'flex', gap: 32, alignItems: 'flex-start' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 20, overflowX: 'auto' }}>
              {workspaces.map(ws => (
                <button key={ws._id} onClick={() => { setActiveWs(ws); api.get(`/projects?workspaceId=${ws._id}`).then(r => setProjects(r.data)); }}
                  className="btn btn-sm" style={{ background: activeWs?._id === ws._id ? 'var(--primary)' : 'var(--bg-surface)', color: activeWs?._id === ws._id ? 'var(--bg-surface)' : 'var(--text-primary)', border: '1px solid var(--sidebar-border)', whiteSpace: 'nowrap' }}>
                  {ws.name}
                </button>
              ))}
            </div>
          ...
          {projects.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 60, border: '2px dashed var(--sidebar-border)', borderRadius: 'var(--radius-lg)', color: 'var(--text-secondary)' }}>
                  ...
                  <div className="card" style={{ cursor: 'pointer', transition: 'all 0.15s', borderTop: `3px solid ${p.colourLabel}` }}
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                      <div>
                        ...
                        <span style={{ fontSize: 11, color: p.status === 'active' ? 'var(--success)' : 'var(--text-tertiary)', fontWeight: 500, textTransform: 'uppercase' }}>{p.status}</span>
                      </div>
                    </div>
                    <p style={{ color: 'var(--text-secondary)', fontSize: 13, marginBottom: 12, lineHeight: 1.4 }}>{p.description || 'No description'}</p>
          ...
          {/* Activity Feed */}
          <div style={{ width: 320, flexShrink: 0, background: 'var(--bg-surface)', border: '1px solid var(--sidebar-border)', borderRadius: 'var(--radius-lg)', padding: 20 }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16 }}>Recent Activity</h3>
            {activities.length === 0 ? (
              <div style={{ fontSize: 13, color: 'var(--text-tertiary)', textAlign: 'center', padding: '20px 0' }}>No recent activity</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {activities.slice(0, 8).map(act => (
                  act.projectId ? (
                    <Link to={`/project/${act.projectId._id || act.projectId}`} key={act._id} style={{ textDecoration: 'none' }}>
                      <div className="activity-item" style={{ display: 'flex', gap: 12, padding: '8px', borderRadius: '8px', transition: 'background 0.2s', cursor: 'pointer' }} onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-base)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 2 }}>{new Date(act.createdAt).toLocaleString()}</div>
                        </div>
                      </div>
                    </Link>
                  ) : (
                    <div key={act._id} style={{ display: 'flex', gap: 12, padding: '8px' }}>
                      <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--primary)', marginTop: 6 }} />
                      <div>
                        <div style={{ fontSize: 13, color: 'var(--text-primary)' }}>
                          <span style={{ fontWeight: 600 }}>{act.actorId?.name}</span> {act.type.replace(':', ' ')} {act.payload?.title || act.payload?.pageId || ''}
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowNewProject(false)}>
          <div className="modal" style={{ padding: 24 }}>
            ...
            <form onSubmit={createProject} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Project name *</label>
              ...
              <div>
                <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Tech stack (comma-separated)</label>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {colors.map(c => (
                    <button key={c} type="button" onClick={() => setNewProject(p => ({ ...p, colourLabel: c }))}
                      style={{ width: 28, height: 28, borderRadius: '50%', background: c, border: newProject.colourLabel === c ? '3px solid var(--text-primary)' : '2px solid transparent', cursor: 'pointer' }} />
                  ))}
                </div>
              </div>
