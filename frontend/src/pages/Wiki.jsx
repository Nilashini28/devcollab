...
export default function Wiki() {
  ...
  function renderMarkdown(text) {
    ...
    return text
      ...
      .replace(/\*(.+?)\*/g, '<em>$1</em>')
      .replace(/`(.+?)`/g, '<code style="background:var(--bg-base);padding:1px 5px;border-radius:3px;font-size:12px;font-family:monospace">$1</code>')
      .replace(/```[\w]*\n([\s\S]*?)```/g, '<pre style="background:var(--bg-base);padding:12px;border-radius:6px;overflow:auto;font-size:12px;font-family:monospace;margin:10px 0">$1</pre>')
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 56px)', overflow: 'hidden' }}>
      {/* Sidebar */}
      <div style={{ width: 240, borderRight: '1px solid var(--sidebar-border)', display: 'flex', flexDirection: 'column', background: 'var(--bg-surface)' }}>
        <div style={{ padding: 12, borderBottom: '1px solid var(--sidebar-border)', display: 'flex', gap: 6 }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '8px 0' }}>
          {loading ? [1,2,3].map(i => <div key={i} className="skeleton" style={{ height: 36, margin: '4px 8px', borderRadius: 6 }} />) :
            ...
            filtered.map(p => (
              <button key={p._id} onClick={() => loadPage(p._id)} className="nav-item" style={{ width: '100%', borderRadius: 0, background: selected?._id === p._id ? 'var(--primary-light)' : 'transparent', color: selected?._id === p._id ? 'var(--primary)' : 'var(--text-primary)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {!selected ? (
          ...
          <>
            {/* Toolbar */}
            <div style={{ padding: '10px 20px', background: 'var(--bg-surface)', borderBottom: '1px solid var(--sidebar-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
              {editing ? (
            ...
            <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                  ...
                  <>
                    {aiSummary && (
                      <div style={{ marginBottom: 20, background: 'var(--info-light)', border: '1px solid var(--info)', borderRadius: 'var(--radius)', padding: 14 }}>
                        ...
                        <button onClick={() => setAiSummary(null)} style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 8 }}>Dismiss</button>
                      </div>
                    )}
                    {editing ? (
                      <textarea value={content} onChange={e => setContent(e.target.value)}
                        style={{ width: '100%', minHeight: 'calc(100vh - 200px)', border: '1.5px solid var(--sidebar-border)', borderRadius: 'var(--radius)', padding: 16, fontFamily: 'monospace', fontSize: 13, resize: 'none', lineHeight: 1.7, background: 'var(--bg-surface)' }}
              ...
              {showHistory && (
                <div style={{ width: 260, borderLeft: '1px solid var(--sidebar-border)', overflow: 'auto', background: 'var(--bg-surface)' }}>
                  <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--sidebar-border)', fontWeight: 600, fontSize: 13 }}>Version History</div>
                  {selected.versions?.length === 0 ? (
                    <div style={{ padding: 20, color: 'var(--text-tertiary)', fontSize: 13 }}>No versions yet</div>
                  ) : (
                    [...(selected.versions || [])].reverse().map((v, i) => (
                      <div key={i} style={{ padding: '10px 14px', borderBottom: '1px solid var(--sidebar-border)' }}>
                        <div style={{ fontSize: 12, fontWeight: 600 }}>v{selected.versions.length - i}</div>
                        <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{new Date(v.savedAt).toLocaleString()}</div>
