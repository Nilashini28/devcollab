    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', padding: 40 }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 16 }}>Start free. Upgrade when your team needs more.</p>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, maxWidth: 680, margin: '0 auto' }}>
          {plans.map(plan => (
            <div key={plan.name} className="card" style={{ border: plan.highlight ? `2px solid var(--primary)` : '1px solid var(--sidebar-border)', position: 'relative', borderRadius: 16 }}>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ marginTop: 8 }}>
                  ...
                  <span style={{ color: 'var(--text-tertiary)', fontSize: 13, marginLeft: 4 }}>{plan.period}</span>
                </div>
              </div>
      ...
      {showUpgrade && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowUpgrade(false)}>
          <div className="modal" style={{ padding: 28, maxWidth: 420 }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: 13, marginBottom: 20 }}>All AI features, unlimited everything.</p>
            <form onSubmit={handleUpgrade} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>Card number</label>
                <input value={card.number} onChange={e => setCard(p => ({ ...p, number: e.target.value }))} placeholder="4242 4242 4242 4242" />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>Expiry</label>
                  <input value={card.expiry} onChange={e => setCard(p => ({ ...p, expiry: e.target.value }))} placeholder="MM/YY" />
                </div>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>CVC</label>
                  <input value={card.cvc} onChange={e => setCard(p => ({ ...p, cvc: e.target.value }))} placeholder="123" />
                </div>
              </div>
              <div style={{ background: 'var(--bg-base)', borderRadius: 8, padding: '10px 14px', fontSize: 13, display: 'flex', justifyContent: 'space-between' }}>
              ...
              <p style={{ fontSize: 11, color: 'var(--text-tertiary)', textAlign: 'center' }}>Demo mode — no real charge</p>
            </form>
          </div>
        </div>
      )}
