    ...
    <div style={{ display: 'flex' }}>
      ...
      <div className="sidebar">
        ...
        <div style={{ padding: '12px 8px', flex: 1, overflow: 'auto' }}>
          ...
          {projects.length > 0 && (
            <>
              <div style={{ margin: '12px 0 4px', padding: '0 12px', fontSize: 11, fontWeight: 600, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Projects</div>
              {projects.map(p => (
                <div key={p._id}>
          ...
          {activeProjectId && devMindAlerts.length > 0 && (
            <div style={{ margin: '12px 0 4px', padding: '0 12px', fontSize: 11, fontWeight: 600, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>DevMind</div>
          )}
          {activeProjectId && devMindAlerts.length > 0 && (
        ...
        <div style={{ padding: '12px 8px', borderTop: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', marginTop: 4 }}>
            ...
            <button onClick={() => { logout(); navigate('/login'); }} style={{ color: 'var(--text-3)', fontSize: 16, padding: 2 }} title="Logout">↩</button>
          </div>
        </div>
      </div>
      ...
      <div className="main-content" style={{ flex: 1 }}>
        ...
        <div style={{ height: 56, background: 'var(--surface)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 20px', gap: 12, position: 'sticky', top: 0, zIndex: 50 }}>
          <div style={{ position: 'relative' }}>
            ...
            {showNotifs && (
              <div style={{ position: 'absolute', right: 0, top: '100%', width: 320, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-lg)', zIndex: 200, maxHeight: 400, overflow: 'auto' }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', fontWeight: 600 }}>Notifications</div>
                {notifications.length === 0 ? <div style={{ padding: 20, color: 'var(--text-3)', textAlign: 'center' }}>All caught up! 🎉</div> : notifications.slice(0, 10).map(n => (
                  <div key={n._id} style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', opacity: n.read ? 0.6 : 1, background: n.read ? 'transparent' : 'var(--primary-light)' }}>
                    <div style={{ fontSize: 13 }}>{n.message}</div>
                    <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>{new Date(n.createdAt).toRelativeDateString?.() || new Date(n.createdAt).toLocaleDateString()}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        {showDevMind && devMindAlerts.length > 0 && (
          <div style={{ margin: '16px 20px 0', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div style={{ fontWeight: 700, fontSize: 15 }}>🧠 DevMind Alerts</div>
              <button onClick={() => setShowDevMind(false)} style={{ color: 'var(--text-3)' }}>✕</button>
            </div>
